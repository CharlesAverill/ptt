...
let rec synth (gamma : typctx) (t : sterm) : (typed_term, string) result =
  ...
  | _ -> fail (Printf.sprintf "Cannot synthesize type for term %s" (string_of_sterm t))

(** Check that the type of [t] matches [ty] under [gamma], and return
    the type-erased version if so *)
...
let erase (t : sterm) : (term, string) result =
  let* (t', _) = typecheck t in return t'
