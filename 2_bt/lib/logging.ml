...
let _log log_level fmt =
  ...
  then Printf.ksprintf (fun _ -> ()) fmt
    ...
    Printf.ksprintf
      (fun msg ->
        let stream =
          if log_level = Log_Debug || log_level = Log_Info then stdout
          else stderr
        in
        Printf.fprintf stream "%s%s %s\n" (string_of_log log_level) ansi_reset
          msg;
        flush stream)
      fmt
...
(** Parse a log level name into a {!log_type}, raising [Failure] if the name is
    not recognized *)
let log_of_string = function
  | "debug" -> Log_Debug
  | "info" -> Log_Info
  | "warning" -> Log_Warning
  | "error" -> Log_Error
